...
public class App
{
    public static void testBoardRep()
    {
        ...
        {
            if (!new Board(tests[i]).toString().equals(tests[i]))
            {
                System.out.println("Failure on " + tests[i]);
                return;
            }
        }
    ...
    public static void testGenerateMoves()
    {
        ...
        for (int i = 0; i < tests.length; i++)
        {
            ...
            if (moves.size() != answers[i])
            {
                System.out.println("Failure on " + i + ", outputted " + moves.size());
            }
        }
        System.out.println("Success");
    }
